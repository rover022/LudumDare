...
class Menu extends Phaser.State {
    ...
    private bun: Phaser.Sprite;
    private drill: Phaser.Sprite;
    private sub: Phaser.Text;
    private title: Phaser.Text;
    private press: Phaser.Text;
    private start: Phaser.Key;
    ...
    create() {
        ...
        if (language === 'zh') {
            ...
            str_sub = "Conejito de\nPerforaci\u00F3n";
            str_prs = "Presione Inicio";
            small = true;
        }
        else if (language === 'fr') {
            str_sub = "FORAGE\nLAPIN";
            str_prs = "Espace Presse";
            small = true;
        }
        else if (language === 'de') {
            str_sub = "BOHRER\nHASE";
        ...
        let xPos: number[] = [25, 50, 75];
        let yPos: number[] = [240, 300, 360];
        randArr(xPos);
        randArr(yPos);
        for (let i: number = 0; i < 3; i++) {
            let a_x: number = xPos[i];
            let a_y: number = yPos[i];

            let apple = this.game.add.sprite(a_x, a_y, 'images');
            ...
            let eaten: Phaser.Sprite = this.game.add.sprite(this.game.rnd.integerInRange(this.game.width / 2 + 16, this.game.width), this.game.height - 65, 'images');
